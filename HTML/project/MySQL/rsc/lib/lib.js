...
document.addEventListener('visibilitychange', function () {
    if (document.visibilityState == 'hidden') {
        ...
        let s4;
        let s5;
        document.querySelector('title').innerText = '悄悄地';
        s1 = setTimeout(function () {
            document.querySelector('title').innerText = '悄悄地你切走了';
            s2 = setTimeout(function () {
                document.querySelector('title').innerText = '正如你悄悄地来';
                s3 = setTimeout(function () {
                    document.querySelector('title').innerText = '你动一动手指';
        ...
        document.addEventListener('visibilitychange', function () {
            document.visibilityState == 'visible' && (function () {
                ...
                clearTimeout(s4);
                clearTimeout(s5);
                document.querySelector('title').innerText = '你回来啦！';
                setTimeout(function () {
...
function newStyle_API_Ver000(arr, elemId) {
    arr && (function () {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            function ctn() {
                /*     if (key.indexOf('CTN') === -1) {} else {
                        // console.log(key);
                        console.log(key.indexOf('CTN'));

                        let k = key.slice(key.indexOf('CTN') + 1, key.length + 1);
                        console.log(k);
                        // k.shift();
                        // console.log(k);
                        newStyle_API_Ver000(k, elemId);
                    } */
            }
