...
let rn = ((Math.random() + 77) * (Math.random() + 37)) / 13 * ((Math.random() + 17) * ((Math.random() + 23) / 3))

sessionStorage.setItem('Q_KVM_RN', sessionStorage.getItem('Q_KVM_RN') + ',' + Math.floor(((Math.random() + 985) * 211) * ((Math.random() + 211) * 985)));
...
function Q_KV_module(arr, ns_div_modulePar) {
    Q_KV_module_par(arr, ns_div_modulePar);
}

function Q_KV_module_par(arr, ns_div_modulePar) {
    // 模块父级
    ...
    // 父级 attribute
    div_modulePar.setAttribute("id", 'Q_KVM_wrap_' + document.querySelector('title').innerText + Q_KVM_index + '_rn_' + rn);

    // 子集 attribute
    ul_contentListPar.setAttribute("id", 'Q_KVM_list_' + Q_KVM_index + '_rn_' + rn);
    ...
    newStyle_API_Ver000(ns_div_modulePar, div_modulePar);
...
function Q_KV_module_contentList(valueArr) {
    // 获取父级
    let par = document.getElementById('Q_KVM_list_' + Q_KVM_index + '_rn_' + rn);
    ...
    function setAnswerInner(para, jd) {
        if (jd == 'list') {
            ...
            if (typeof para == 'object') {
                for (let ib = 0; ib < para.length; ib++) {
                    p_show_answer.innerHTML = para.toString().replace(/,/g, '<br><span style="margin-left: 2rem;"><span>');
                }
            }
        }
    }
    ...
    li_contentList.setAttribute('id', li_contentList_name + '_rn_' + rn);
